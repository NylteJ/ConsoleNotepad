...
1. 添加 Ctrl + H （替换）
2. 增加编码选择功能，并能检测当前编码是否不正确
3. 添加自动保存
4. 优化查找 / 替换，支持动态区分大小写、全字匹配、正则表达式等等，并能只使用 \n 匹配 \r
5. 增加行号、文件名、光标位置等的显示
6. 支持通过控制台参数直接打开文件等
7. 添加配置文件，允许简单地配置各项参数（或者干脆做一个设置界面）
8. 支持不同编码的自动识别
9. 添加一个历史记录页面，可以借此更好地撤回（其实这个还挺好做的）
10. 增加更多的高亮提示等（比如查找 / 替换时高亮所有选择结果），这一条涉及的底层比较多所以放后面了